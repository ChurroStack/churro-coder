...
- **L3 dispatcher action prompts (both harnesses):** `dispatchBuildPlan` (`use-harness-send-dispatcher.ts`) embeds `Sub-chat id: <X>` into its natural-language instruction. `dispatchReview` does NOT — it dispatches the harness's real native review command (`/code-review high` on claude-cli, `/review` on codex-cli) with no MCP/subChatId instruction attached: these run as local commands/review-mode turns with no opportunity for the model to call an MCP tool mid-command, so persistence happens out-of-band via CLI-ingest's `review` side-effect (`cli-session/jsonl-mapper.ts` + `ingester.ts`), which already knows the subChatId from its own per-subChat lifecycle.
...
- **xterm instances:** one `Terminal` component per paneId; xterm/sizer/serializeAddon refs live in `useRef` inside `terminal.tsx` and are never shared across mounts. The `TerminalSizer` (`terminal-sizing.ts`) that owns the measure→fit→resize lifecycle is likewise one-per-mount. A future refactor that reuses a single xterm canvas (or sizer) across panels would reintroduce the cross-talk class — push back on it in review.
