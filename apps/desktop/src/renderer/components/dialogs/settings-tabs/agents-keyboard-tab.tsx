...
function ShortcutDetailPanel({
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8">
      ...
      <button
        ...
        {(() => {
          ...
          if (isRecording) {
            ...
            return <span className="text-sm text-muted-foreground animate-pulse">Press keys...</span>;
          }
      ...
      {conflictMessage && <p className="text-xs text-red-500 mt-3 animate-pulse">{conflictMessage}</p>}
      ...
      <div className="mt-6 h-8 flex items-center justify-center">
        {isCustom ? (
          ...
          <p className="text-xs text-muted-foreground text-center">Click to record a new shortcut</p>
        )}
      </div>
    </div>
  );
...
function EmptyDetailPanel() {
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center">
      ...
      <p className="text-sm text-muted-foreground">Select a shortcut to customize</p>
    </div>
  );
}
...
export function AgentsKeyboardTab() {
  ...
  const allActionIds = useMemo(
    () =>
      (['general', 'workspaces', 'agents'] as ShortcutCategory[]).flatMap((cat) =>
        filteredShortcuts[cat].map((a) => a.id)
      ),
    [filteredShortcuts]
  );
  ...
  const hasCustomHotkeys = useMemo(() => Object.keys(customHotkeys.bindings).length > 0, [customHotkeys]);
  ...
  const checkConflict = useCallback(
    (hotkey: string, currentActionId: ShortcutActionId): ShortcutAction | null => {
      ...
      <ResizableSidebar
        ...
        <div
          ...
          <div
            ...
            {totalShortcuts === 0 ? (
              <div className="text-center py-8 text-sm text-muted-foreground">No shortcuts found</div>
            ) : (
