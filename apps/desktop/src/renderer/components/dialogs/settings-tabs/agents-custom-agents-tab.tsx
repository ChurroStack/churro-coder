...
function AgentDetail({
  ...
  const hasChanges =
    description !== agent.description || prompt !== agent.prompt || model !== (agent.model || 'inherit');

  const handleSave = useCallback(() => {
    if (description !== agent.description || prompt !== agent.prompt || model !== (agent.model || 'inherit')) {
      onSave({
        description,
        prompt,
  ...
  const handleModelChange = useCallback(
    (value: string) => {
      ...
      if (description !== agent.description || prompt !== agent.prompt || value !== (agent.model || 'inherit')) {
        onSave({
          description,
          prompt,
...
export function AgentsCustomAgentsTab({ mode = GLOBAL_SCOPE_MODE }: { mode?: SettingsScopeMode } = {}) {
  ...
  const handleCreate = useCallback(
    async (data: { name: string; description: string; prompt: string; model?: string; source: 'user' | 'project' }) => {
      try {
