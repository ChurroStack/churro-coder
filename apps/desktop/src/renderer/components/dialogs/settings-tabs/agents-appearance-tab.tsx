...
function ThemePreviewBox({
  ...
  return (
    <div
      className={cn('flex-shrink-0 flex items-center justify-center font-semibold', sizeClasses, className)}
      style={{
        backgroundColor: bgColor,
        boxShadow: 'inset 0 0 0 0.5px rgba(128, 128, 128, 0.3)'
      }}>
      {/* Accent dot to the left of text */}
      <div className={cn('rounded-full flex-shrink-0', dotSize)} style={{ backgroundColor: accentColor }} />
...
export function AgentsAppearanceTab() {
  ...
  const applyTheme = useCallback(
    (themeId: string | null) => {
      ...
      // Check in both builtin and imported themes
      const theme = BUILTIN_THEMES.find((t) => t.id === themeId) || importedThemes.find((t) => t.id === themeId);
      if (theme) {
  ...
  const importedDarkThemes = useMemo(() => importedThemes.filter((t) => t.type === 'dark'), [importedThemes]);
  const importedLightThemes = useMemo(() => importedThemes.filter((t) => t.type === 'light'), [importedThemes]);
    ...
    <div className="p-6 space-y-6 flex-1 overflow-y-auto">
      ...
      {!isNarrowScreen && (
        <div className="flex flex-col space-y-1.5 text-center sm:text-left">
          ...
          <p className="text-xs text-muted-foreground">Customize the look and feel of the interface</p>
        </div>
      )}
      ...
      <div className="bg-background rounded-lg border border-border overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex flex-col space-y-1">
            ...
            <span className="text-xs text-muted-foreground">Select or customize your interface color scheme</span>
          </div>
            ...
            <SelectTrigger className="w-auto px-2">
              <div className="flex items-center gap-2 min-w-0 -ml-[3px]">
                {isSystemMode ? (
                  <>
                    <ThemePreviewBox
                      theme={resolvedTheme === 'dark' ? (systemDarkTheme ?? null) : (systemLightTheme ?? null)}
                    />
            ...
            <SelectContent className="max-h-[300px]">
              ...
              {importedThemes.length > 0 && (
                <>
                  <SelectSeparator />
                  <SelectGroup>
                    <SelectLabel className="text-xs text-muted-foreground px-2">From editors</SelectLabel>
            ...
            <motion.div
              ...
              <div className="flex items-center justify-between p-4 border-t border-border">
                <div className="flex flex-col space-y-1">
                  ...
                  <span className="text-xs text-muted-foreground">Theme to use for light system appearance</span>
                </div>
              ...
              <div className="flex items-center justify-between p-4 border-t border-border">
                <div className="flex flex-col space-y-1">
                  ...
                  <span className="text-xs text-muted-foreground">Theme to use for dark system appearance</span>
                </div>
      ...
      <div className="bg-background rounded-lg border border-border overflow-hidden">
        <div className="flex items-center justify-between p-4">
          <div className="flex flex-col space-y-1">
            ...
            <span className="text-xs text-muted-foreground">Show project icon in the sidebar workspace list</span>
          </div>
