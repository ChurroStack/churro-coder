      ...
      <div className="max-w-2xl mx-auto p-6 space-y-5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={cn(
                  ...
                  item.kind === 'skill' ? 'bg-blue-500/10 text-blue-500' : 'bg-orange-500/10 text-orange-500'
                )}>
        ...
        <div className="space-y-1.5">
          ...
          <div className="px-3 py-2 text-sm bg-muted/50 border border-border rounded-lg">
            <code className="text-xs text-foreground">{item.kind === 'skill' ? `@${item.name}` : `/${item.name}`}</code>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {!isReadOnly && (
              <Tooltip>
                ...
                <TooltipContent>{viewMode === 'rendered' ? 'Edit markdown' : 'Preview markdown'}</TooltipContent>
              </Tooltip>
            )}
          </div>
...
function CreateItemForm({
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="max-w-2xl mx-auto p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">{kind === 'skill' ? 'New Skill' : 'New Command'}</h3>
          <div className="flex items-center gap-2">
...
function SidebarListItem({
  ...
  return (
    <button
      ...
      {item.description && (
        <div className="text-[11px] text-muted-foreground truncate mt-0.5 pl-[18px]">{item.description}</div>
      )}
    </button>
  );
}
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <AlertDialog
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete {deletingItem?.kind === 'skill' ? 'Skill' : 'Command'}</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{deletingItem?.name}</strong>? This will remove the file from disk
