...
export interface FileMentionOption {
  ...
  source?: 'user' | 'project' | 'plugin' | 'builtin'; // skill/agent source
      ...
      if (cursorInThisNode) {
        ...
        if (localAtIdx !== -1) {
          ...
          if (isStandaloneAt && globalAtIdx > atIndex) {
            const afterAt = textBeforeCursor.slice(textBeforeCursor.lastIndexOf('@') + 1);
        ...
        for (let i = 0; i < textBeforeInNode.length; i++) {
          if (textBeforeInNode[i] === '/') {
            ...
            const charBefore =
              globalSlashIdx === 0 ? null : (serialized + textBeforeInNode.slice(0, i)).charAt(globalSlashIdx - 1);
...
export const AgentsMentionsEditor = memo(
  forwardRef<AgentsMentionsEditorHandle, AgentsMentionsEditorProps>(function AgentsMentionsEditor(
    {
      initialValue,
      onTrigger,
      onCloseTrigger,
    ...
    const resolveMention = useCallback((id: string): FileMentionOption | null => {
    ...
    const handleInput = useCallback(() => {
      ...
      const runTriggerDetection = () => {
        ...
        const { textBeforeCursor, atPosition, atIndex, slashPosition, slashIndex } = walkTreeOnce(
          editorRef.current,
          range
        );
