...
import { useCallback, useEffect, useLayoutEffect, useMemo, useRef, useState, createElement, memo } from 'react';
    ...
    <div className="flex flex-col gap-1 min-w-[220px]">
      {parts.map((part, index) => {
        ...
        return (
          <div
            key={index}
            className={cn('flex items-center gap-1.5 text-xs', isLast ? 'text-foreground' : 'text-muted-foreground')}
            style={{ paddingLeft: `${index * 20}px` }}>
            <FolderOpenIcon
              className={cn('h-3.5 w-3.5 flex-shrink-0', isLast ? 'text-foreground/70' : 'text-muted-foreground')}
            />
 ...
 */
function sortFilesByRelevance<T extends { label: string; path?: string }>(files: T[], searchText: string): T[] {
...
function renderTooltipContent(option: FileMentionOption) {
  ...
  if (option.type === 'skill' || option.type === 'agent') {
    return (
      <div className="flex flex-col gap-1.5 w-full overflow-hidden">
        {option.description && <p className="text-xs text-muted-foreground break-words">{option.description}</p>}
        {option.model && <div className="text-xs text-muted-foreground">Model: {option.model}</div>}
        {option.tools && option.tools.length > 0 && (
          <div className="text-xs text-muted-foreground break-words">Tools: {option.tools.join(', ')}</div>
        )}
        <div className="text-[10px] text-muted-foreground/70 font-mono truncate w-full">{option.path}</div>
      </div>
    );
  }
  ...
  // Files - just path
  return <div className="text-xs text-muted-foreground font-mono truncate w-full">{option.path}</div>;
}
...
export const AgentsFileMention = memo(function AgentsFileMention({
  ...
  const changedFilePaths = useMemo(() => new Set(changedFiles.map((f) => f.filePath)), [changedFiles]);
    ...
    return skills
      .filter(
        (skill) =>
          ...
          matchesMultiWordSearch(skill.name, searchLower) || matchesMultiWordSearch(skill.description, searchLower)
      )
      .map((skill) => ({
  ...
  const agentOptions: FileMentionOption[] = useMemo(() => {
    ...
    return customAgents
      .filter(
        (agent) =>
          // Multi-word search: all words must match in name OR description
          matchesMultiWordSearch(agent.name, searchLower) || matchesMultiWordSearch(agent.description, searchLower)
      )
      .map((agent) => ({
  ...
  const allToolOptions: FileMentionOption[] = useMemo(() => {
    ...
    const connectedServers = sessionInfo.mcpServers.filter((s) => s.status === 'connected');
  ...
  return createPortal(
    <TooltipProvider delayDuration={300}>
      <div
        ...
        {/* Error state */}
        {error && (
          <div className="h-7 px-1.5 mx-1 flex items-center text-xs text-muted-foreground">Error loading files</div>
        )}
            ...
            <>
              ...
              {(isInSubpage || debouncedSearchText) && (
                <div className="px-2.5 py-1.5 mx-1 text-xs font-medium text-muted-foreground flex items-center gap-1.5">
                  ...
                  {isFetching && !isLoading && <IconSpinner className="h-2.5 w-2.5" />}
                </div>
              )}
              {options.map((option, index) => {
                ...
                const itemContent = (
                  <div
                    ...
                    <span className="flex items-center gap-1 w-full min-w-0">
                      ...
                      {(option.additions || option.deletions) && (
                        <span className="shrink-0 flex items-center gap-1 text-[10px] font-mono">
                          {option.additions ? <span className="text-green-500">+{option.additions}</span> : null}
                          {option.deletions ? <span className="text-red-500">-{option.deletions}</span> : null}
                        </span>
                      )}
                    ...
                    {isCategory && <ChevronRight className="ml-auto h-3.5 w-3.5 text-muted-foreground flex-shrink-0" />}
                  </div>
                );
