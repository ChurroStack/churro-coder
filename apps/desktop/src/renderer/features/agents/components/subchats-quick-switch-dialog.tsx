    ...
    <div
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="pt-0.5 relative flex-shrink-0 h-4 w-4">
          {mode === 'plan' ? (
            <PlanIcon className={cn('w-4 h-4', isSelected ? 'text-primary-foreground' : 'text-muted-foreground')} />
          ) : (
            <AgentIcon className={cn('w-4 h-4', isSelected ? 'text-primary-foreground' : 'text-muted-foreground')} />
          )}
          ...
          {(isLoading || hasUnseenChanges) && (
            <div
              ...
              {isLoading ? (
                <IconSpinner
                  className={cn('w-2.5 h-2.5', isSelected ? 'text-primary-foreground' : 'text-muted-foreground')}
                />
              ) : (
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-0.5">
          ...
          <div className="flex items-center gap-1.5 text-[11px]">
            <span className={cn(isSelected ? 'text-primary-foreground/60' : 'text-muted-foreground/60')}>
              {timeAgo}
            </span>
            {stats && (
              <>
                <span className={cn(isSelected ? 'text-primary-foreground/40' : 'text-muted-foreground/40')}>·</span>
                <span className={cn(isSelected ? 'text-primary-foreground/60' : 'text-muted-foreground/60')}>
                ...
                {(stats.additions > 0 || stats.deletions > 0) && (
                  <>
                    <span
                      className={cn(isSelected ? 'text-primary-foreground/80' : 'text-green-600 dark:text-green-400')}>
                      +{stats.additions}
                    </span>
                    <span className={cn(isSelected ? 'text-primary-foreground/80' : 'text-red-600 dark:text-red-400')}>
                      -{stats.deletions}
                    </span>
                  </>
                )}
