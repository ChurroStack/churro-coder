...
export function AgentsHelpPopover({
  ...
  useEffect(() => {
    let cancelled = false
    window.desktopApi
      .signedFetch("https://www.churrostack.com/api/changelog/desktop?per_page=3")
      .then((result) => {
        if (cancelled) return
  ...
  */
  ...
  const handleChangelogClick = () => {
    window.desktopApi.openExternal("https://www.churrostack.com/agents/changelog")
  }

  const handleReleaseClick = (version: string) => {
    window.desktopApi.openExternal(
      `https://www.churrostack.com/agents/changelog#${version}`,
    )
  }
  */
