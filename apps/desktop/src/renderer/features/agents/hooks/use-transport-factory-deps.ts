...
export function useTransportFactoryDeps(
  config: UseTransportFactoryDepsConfig,
): TransportFactoryDeps<Chat<any>> {
  const {
    chatId,
    worktreePath,
    projectPath,
  ...
  return useMemo<TransportFactoryDeps<Chat<any>>>(
    () => ({
      ...
      isStreaming: (id) => {
        ...
        const existingStatus = (existing as { status?: string } | undefined)
          ?.status
      ...
      getExistingProvider: (chat) =>
        (chat as { transport?: unknown })?.transport instanceof CodexChatTransport
          ? "codex"
          : "claude-code",
