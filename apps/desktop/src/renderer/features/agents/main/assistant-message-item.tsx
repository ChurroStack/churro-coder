...
function normalizeAcpParts(parts: any[]): any[] {
  return parts.map((part) => {
    ...
    const isAcpPart =
      part.input?.toolName || part.type.includes(' ') || part.type === 'tool-acp.acp_provider_agent_dynamic_tool';
    if (!isAcpPart) return part;

    const partInput = part.input && typeof part.input === 'object' ? (part.input as Record<string, any>) : {};
    ...
    if (verb) {
      ...
      args = partInput.args && typeof partInput.args === 'object' ? (partInput.args as Record<string, any>) : partInput;
    }
...
export const AssistantMessageItem = memo(function AssistantMessageItem({
  ...
  const contentParts = useMemo(() => messageParts.filter((p: any) => p.type !== 'step-start'), [messageParts]);
  ...
  const { nestedToolsMap, nestedToolIds, orphanTaskGroups, orphanToolCallIds, orphanFirstToolCallIds } = useMemo(() => {
    ...
    const taskPartIds = new Set(
      messageParts.filter((p: any) => isSubagentDispatchType(p.type) && p.toolCallId).map((p: any) => p.toolCallId)
    );
  ...
  return (
    <div data-assistant-message-id={message.id} className="group/message w-full mb-4">
        ...
        {(() => {
          ...
          return grouped.map((part: any, idx: number) => {
            if (part.type === 'exploring-group') {
              ...
              const isGroupStreaming = isStreaming && isLastMessage && isLast;
              return (
                <AgentExploringGroup key={idx} parts={part.parts} chatStatus={status} isStreaming={isGroupStreaming} />
              );
            }
            if (part.type === 'task-group') {
              const isLast = idx === grouped.length - 1;
            ...
            {onFork && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="min-w-[160px]">
                  <DropdownMenuItem onClick={() => onFork(message.id)}>Fork from here</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
          </div>
      ...
      {(!isStreaming || !isLastMessage) && (
        <GitActivityBadges parts={messageParts} metadata={msgMetadata} chatId={chatId} subChatId={subChatId} />
      )}
