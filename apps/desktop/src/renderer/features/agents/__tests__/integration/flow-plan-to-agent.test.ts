...
function makeRealisticDeps(opts: {
  subChatId: string
  initialProvider: ProviderId
}): { deps: PlanApprovalDeps; orchestration: OrchestrationResult } {
  ...
  const deps: PlanApprovalDeps = {
    readPreviousProvider: (subChatId) =>
      appStore.get(subChatProviderOverrideAtomFamily(subChatId)) ?? "claude-code",
...
describe("L4 integration — plan → agent same-provider Claude→Claude (PR #36, #38, #44, #45, #51)", () => {
  test("approve flips mode atom to agent and applies sonnet model synchronously", async () => {
    ...
    expect(orchestration.persistCalls).toEqual([
      { subChatId, mode: "agent", exitPlan: true },
    ])
