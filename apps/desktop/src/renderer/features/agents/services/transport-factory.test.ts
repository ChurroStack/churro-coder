...
function makeHarness(opts: {
  ...
  const deps: TransportFactoryDeps<FakeChat> = {
    ...
    readPersistedMessages: vi.fn(
      (subChatId: string) => persisted.get(subChatId) ?? opts.persistedMessages ?? [],
    ),
    isStreaming: vi.fn(
      (subChatId: string) => isStreamingMap.get(subChatId) ?? opts.isStreaming ?? false,
    ),
    hasQueue: vi.fn(
      (subChatId: string) => hasQueueMap.get(subChatId) ?? opts.hasQueue ?? false,
    ),
  ...
  return { deps, store, persisted, isStreamingMap, hasQueueMap, staleRuntime: !!opts.isStaleRuntime, createCalls, deleteCalls }
}
...
describe("getOrCreateChat — KEEP existing transport", () => {
  ...
  test("PR #44 — cross-provider WITH messages → KEEP", () => {
    ...
    const result = getOrCreateChat(
      { subChatId: "sub-1", targetProvider: "codex", targetIsRemote: false },
      h.deps,
    )
    expect(result.action).toEqual({ kind: "keep" })
    expect(h.createCalls).toHaveLength(0)
...
describe("getOrCreateChat — RECREATE order", () => {
  test("deleteExistingChat runs BEFORE createChat (so storeChat overwrites cleanly)", () => {
    ...
    const deleteCallOrder = (h.deps.deleteExistingChat as ReturnType<typeof vi.fn>).mock
      .invocationCallOrder[0]
    const createCallOrder = (h.deps.createChat as ReturnType<typeof vi.fn>).mock
      .invocationCallOrder[0]
