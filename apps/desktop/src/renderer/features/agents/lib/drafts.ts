...
export async function saveNewChatDraftWithAttachments(
  ...
  const draftImages = options?.images?.map(toDraftImage).filter((img): img is DraftImage => img !== null) ?? [];
 ...
 */
function wouldExceedStorageLimit(existingDrafts: GlobalDraftsRaw, newDraft: DraftContent | NewChatDraft): boolean {
