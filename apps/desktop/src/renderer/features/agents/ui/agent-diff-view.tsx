...
import { AlertTriangle, Check, ChevronDown, Columns2, Rows2 } from 'lucide-react';
    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="flex items-center gap-2 p-4 text-sm text-yellow-600 dark:text-yellow-500 bg-yellow-50 dark:bg-yellow-950/30 rounded-md">
          ...
          <span>Failed to render diff for this file. The diff format may be corrupted or truncated.</span>
        </div>
      );
    }
 ...
 */
function reconstructOldContent(newContent: string, patchText: string): string | null {
  try {
...
const validateDiffHunk = (diffText: string): { valid: boolean; reason?: string } => {
...
const fileDiffCardAreEqual = (prev: FileDiffCardProps, next: FileDiffCardProps): boolean => {
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 text-left min-w-0 min-h-[22px]">
          ...
          {(() => {
            ...
            return FileIcon ? <FileIcon className="w-3.5 h-3.5 shrink-0 text-muted-foreground" /> : null;
          })()}

          {/* File name + path + status */}
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <span className="font-medium text-foreground shrink-0">{fileName}</span>
            {dirPath && <span className="text-muted-foreground truncate text-[11px] min-w-0">{dirPath}</span>}
            {isNewFile && <span className="shrink-0 text-[11px] text-emerald-600 dark:text-emerald-400">(new)</span>}
            {isDeletedFile && <span className="shrink-0 text-[11px] text-red-600 dark:text-red-400">(deleted)</span>}
          </div>

          {/* Stats */}
          <span className="shrink-0 font-mono text-[11px] tabular-nums whitespace-nowrap">
            {file.additions > 0 && (
              <span className="mr-1.5 text-emerald-600 dark:text-emerald-400">+{file.additions}</span>
            )}
                    ...
                    <>
                      <ExpandIcon
                        className={cn(
                          ...
                          isFullExpanded ? 'opacity-0 scale-75' : 'opacity-100 scale-100'
                        )}
                      />
                      <CollapseIcon
                        className={cn(
                          'absolute inset-0 w-3.5 h-3.5 text-muted-foreground transition-[opacity,transform] duration-200 ease-out',
                          isFullExpanded ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
                        )}
                      />
                    </>
                  )}
        ...
        {!isCollapsed && !file.isBinary && !hasContent && isLoadingContent && (
              ...
              <button
                ...
                <div
                  className={cn(
                    ...
                    isViewed ? 'bg-primary text-primary-foreground' : 'border border-muted-foreground/40'
                  )}>
  ...
  return (
    <div
      ...
      {!isCollapsed && (
        <div>
          {file.isBinary ? (
            <div className="px-3 py-2 text-xs text-muted-foreground">Binary file diff can't be rendered.</div>
          ) : isLargeDiff ? (
            ...
            <div className="flex items-center gap-2 px-3 py-2 text-xs text-yellow-600 dark:text-yellow-500 bg-yellow-50 dark:bg-yellow-950/30">
              ...
              <span>Diff format appears truncated or corrupted. Unable to render this file's changes.</span>
            </div>
          ) : (
...
interface AgentDiffViewProps {
  ...
  onCollapsedStateChange?: (state: { allCollapsed: boolean; allExpanded: boolean }) => void;
...
export const AgentDiffView = forwardRef<AgentDiffViewRef, AgentDiffViewProps>(function AgentDiffView(
  {
    chatId,
    sandboxId,
    worktreePath,
  ...
  const [isLoadingDiff, setIsLoadingDiff] = useState(
    initialDiff === undefined || (initialDiff === null && !Array.isArray(initialParsedFiles))
  );
  ...
  const isFileViewed = useCallback(
    (fileKey: string, diffText: string): boolean => {
  ...
  const [fileContents, setFileContents] = useState<Record<string, string>>(prefetchedFileContents ?? {});
  ...
  const fileDiffs = useMemo(() => {
    ...
    return validFiles.filter((file) => {
      ...
      return validFilterPaths.some(
        (filterPath) => filePath === filterPath || filePath.endsWith(filterPath) || filterPath.endsWith(filePath)
      );
    });
            ...
            filesToFetch.map(async ({ key, filePath }) => {
              if (isDesktopApp()) {
                ...
                const response = await Promise.race([
                  fetch(`/api/agents/sandbox/${sandboxId}/files?path=${encodeURIComponent(filePath)}`),
  ...
  const handleToggleViewed = useCallback(
    (fileKey: string, diffText: string) => {
      ...
      if (willBeViewed && onSelectNextFile) {
        ...
        if (nextUnviewedFile) {
          ...
          const filePath =
            nextUnviewedFile.newPath && nextUnviewedFile.newPath !== '/dev/null'
  ...
  const prevStatsRef = useRef<{ fileCount: number; additions: number; deletions: number; isLoading: boolean } | null>(
    null
  );
  useEffect(() => {
  ...
  }, [allFileDiffs.length, totalAdditions, totalDeletions, isLoadingDiff, filteredDiffFiles]);
  ...
  useEffect(() => {
    ...
    fileIndex = fileDiffs.findIndex((f) => f.newPath === focusedDiffFile || f.oldPath === focusedDiffFile);
    ...
    if (fileIndex >= 0) {
      ...
      setTimeout(() => {
        ...
        if (container) {
          const fileCard = container.querySelector(`[data-diff-file-path="${focusedDiffFile}"]`) as HTMLElement | null;
  ...
  }, [focusedDiffFile, isLoadingDiff, setFocusedDiffFile, fileDiffs, collapsedByFileKey]);
  ...
  return (
    <div ref={scopeRef} className={cn('flex flex-col bg-background overflow-hidden min-w-0', isMobile ? 'h-full w-full' : 'h-full')}>
      ...
      {/* Content */}
      <div ref={scrollContainerRef} className="relative flex-1 overflow-auto p-2 select-text">
        {/* Sticky cover to hide content scrolling above cards */}
        <div className="sticky top-0 left-0 right-0 h-0 z-20 pointer-events-none" aria-hidden="true">
          <div className="absolute -top-2 left-0 right-0 h-2 bg-background" />
        </div>

        {isLoadingDiff || (isLoadingFileContents && fileDiffs.length === 0) ? (
      ...
      <AlertDialog open={!!discardFilePath} onOpenChange={(open) => !open && setDiscardFilePath(null)}>
        <AlertDialogContent className="w-[340px]">
          <AlertDialogHeader>
            <AlertDialogTitle>Discard changes to "{discardFilePath?.split('/').pop()}"?</AlertDialogTitle>
          </AlertDialogHeader>
