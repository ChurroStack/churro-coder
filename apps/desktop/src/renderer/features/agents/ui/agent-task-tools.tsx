 ...
 */
function extractChangesFromParts(parts: any[], snapshotSubjects?: Map<string, string>): ExtractedChanges {
...
const InProgressIcon = ({ size = 14 }: { size?: number }) => {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="text-muted-foreground flex-shrink-0">
      {/* Outer border circle */}
...
const TaskInfoItem = memo(function TaskInfoItem({
  ...
  return (
    <div className={cn('flex items-start gap-2 px-2.5 py-2', !isLast && 'border-b border-border/30')}>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        {hasBlockedBy && (
          <BlockingTasksList blockedByIds={task.blockedBy!} taskSubjects={taskSubjects} taskSnapshot={taskSnapshot} />
        )}
        {blocksDesc && <p className="text-xs text-muted-foreground/70 mt-0.5">{blocksDesc}</p>}
        {task.owner && <p className="text-xs text-muted-foreground/70 mt-0.5">owner: {task.owner}</p>}
      </div>
    </div>
  );
});
...
const ChangeItem = memo(function ChangeItem({
  ...
  const displayText = change.newStatus === 'in_progress' && change.activeForm ? change.activeForm : change.subject;
    ...
    <div className={cn('flex items-start gap-2 px-2.5 py-2', !isLast && 'border-b border-border/30')}>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        {change.description && change.changeType === 'created' && (
          <p className="text-xs text-muted-foreground/70 mt-0.5 line-clamp-2">{change.description}</p>
        )}
        {hasBlockedBy && (
          <BlockingTasksList
