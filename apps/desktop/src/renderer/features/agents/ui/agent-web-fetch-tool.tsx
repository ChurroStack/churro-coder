    ...
    <div className="rounded-lg border border-border bg-muted/30 overflow-hidden mx-2">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-2">
          <div className="flex items-center gap-1.5 text-xs">
            {isPending ? (
              ...
              <span className="text-destructive">{statusCode ? `Error ${statusCode}` : 'Failed'}</span>
            ) : (
      ...
      {hasContent && isExpanded && (
        <div className="border-t border-border max-h-[300px] overflow-y-auto">
          <pre className="px-2.5 py-2 text-xs text-foreground whitespace-pre-wrap break-words font-mono">{result}</pre>
        </div>
      )}
    </div>
  );
