...
const ProgressCircle = ({
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className={cn('text-muted-foreground', className)}>
      {/* Outer border circle */}
...
const TodoListItem = memo(function TodoListItem({
  ...
  return (
    <div className={cn('flex items-center gap-2 px-2.5 py-1.5', !isLast && 'border-b border-border/30')}>
...
export const AgentTodoTool = memo(function AgentTodoTool({ part, chatStatus, subChatId }: AgentTodoToolProps) {
  ...
  useEffect(() => {
    if (newTodos.length > 0) {
      ...
      const isCreationToolCallLocal =
        currentCreationToolCallId === null || currentCreationToolCallId === part.toolCallId || isNewGenerationLocal;
  ...
  if (newTodos.length === 0 || (isStreaming && !part.input?.todos)) {
    ...
    return (
      <div className="mx-2 sticky z-[5] bg-background" style={{ top: 'calc(var(--user-message-height, 28px) - 29px)' }}>
  ...
  if (changes.type === 'single' && !(alwaysExpandTodoList && isCreationToolCall)) {
    ...
    const titleText =
      change.newStatus === 'in_progress' && change.todo.activeForm ? change.todo.activeForm : change.todo.content;
        ...
        <div className="flex-1 min-w-0 flex items-center gap-1.5">
          <div className="text-xs text-muted-foreground flex items-center gap-1.5 min-w-0">
            ...
            <div className="flex items-center gap-1 text-muted-foreground/60 font-normal truncate min-w-0">
              ...
              {remainingCount > 0 && (
                <span className="text-muted-foreground/60 whitespace-nowrap flex-shrink-0">+{remainingCount} more</span>
              )}
            </div>
          </div>
        </div>
  ...
  const currentTask =
    displayTodos.find((t) => t.status === 'in_progress') || displayTodos.find((t) => t.status === 'pending');
  ...
  return (
    <div
      ...
      <div className="rounded-b-lg border border-border bg-muted/20 shadow-xl shadow-background">
          ...
          <div
            ...
            <div className="flex items-center gap-1.5 min-w-0 flex-1">
              ...
              {!currentTask && completedCount === totalTodos && totalTodos > 0 && (
                <span className="text-xs text-muted-foreground truncate">{displayTodos[totalTodos - 1]?.content}</span>
              )}
            </div>
        ...
        {isExpanded && (
          <div className="max-h-[300px] overflow-y-auto cursor-pointer" onClick={handleCollapse}>
