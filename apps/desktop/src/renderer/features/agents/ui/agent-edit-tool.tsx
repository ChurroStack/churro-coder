...
function useBatchHighlight(
  ...
  const linesKey = useMemo(() => (isStreaming ? '' : lines.map((l) => l.content).join('\n')), [lines, isStreaming]);
...
const DiffLineRow = memo(
  function DiffLineRow({ line, highlightedHtml }: { line: DiffLine; highlightedHtml: string | undefined }) {
    return (
      <div
        className={cn(
          'px-2.5 py-0.5',
          line.type === 'removed' && 'bg-red-500/10 dark:bg-red-500/15 border-l-2 border-red-500/50',
          line.type === 'added' && 'bg-green-500/10 dark:bg-green-500/15 border-l-2 border-green-500/50',
