...
export const AgentMcpToolCall = memo(function AgentMcpToolCall({ part, mcpInfo, chatStatus }: AgentMcpToolCallProps) {
  ...
  const hasExpandableContent = ((part.input && Object.keys(part.input).length > 0) || !!part.output) && !isPending;

  if (isInterrupted && !part.output) {
    return <AgentToolInterrupted toolName={mcpInfo.displayName} subtitle={`via ${mcpInfo.serverName}`} />;
  }
