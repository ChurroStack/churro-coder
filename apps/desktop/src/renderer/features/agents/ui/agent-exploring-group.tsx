...
export const AgentExploringGroup = memo(function AgentExploringGroup({
  ...
  const fileCount = parts.filter((p) => ['tool-Read', 'tool-Grep', 'tool-Glob'].includes(p.type)).length;
      ...
      {isExpanded && (
        <div className="relative mt-1">
          ...
          <div
            ...
            style={
              parts.length > MAX_VISIBLE_TOOLS ? { maxHeight: `${MAX_VISIBLE_TOOLS * TOOL_HEIGHT_PX}px` } : undefined
            }>
            {parts.map((part, idx) => {
