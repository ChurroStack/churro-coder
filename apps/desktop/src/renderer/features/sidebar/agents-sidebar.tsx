    ...
    <div className="relative flex-shrink-0 w-4 h-4">
      ...
      <div
        className={cn(
          ...
          isMultiSelectMode || !showIcon ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'
        )}>
        {renderMainIcon()}
      </div>
      ...
      <ContextMenuTrigger asChild>
        <div
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
              <div className="flex items-center gap-1">
                <span ref={(el) => nameRefCallback(chatId, el)} className="truncate block text-sm leading-tight flex-1">
                  <TypewriterText
                    text={chatName || ''}
              ...
              <div className="flex items-center gap-1 text-[11px] text-muted-foreground/60 min-w-0">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  <span>{formatTime(chatUpdatedAt?.toISOString() ?? new Date().toISOString())}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <ContextMenuContent className="w-48">
        ...
        {isMultiSelectMode && isChecked ? (
          ...
          <>
            {isRemote && (
              <>
                <ContextMenuItem onClick={() => onOpenLocally(chatId)}>Fork Locally</ContextMenuItem>
                <ContextMenuSeparator />
              </>
            )}
            ...
            {chatBranch && <ContextMenuItem onClick={() => onCopyBranch(chatBranch)}>Copy branch name</ContextMenuItem>}
            <ContextMenuSub>
              ...
              <ContextMenuSubContent sideOffset={6} alignOffset={-4}>
                ...
                <ContextMenuItem
                  onClick={() =>
                    exportChat({ chatId: isRemote ? chatId.replace(/^remote_/, '') : chatId, format: 'json', isRemote })
                  }>
                  Download as JSON
                </ContextMenuItem>
                <ContextMenuItem
                  onClick={() =>
                    exportChat({ chatId: isRemote ? chatId.replace(/^remote_/, '') : chatId, format: 'text', isRemote })
                  }>
                  Download as Text
                </ContextMenuItem>
                <ContextMenuSeparator />
                ...
                <ContextMenuItem
                  onClick={() =>
                    copyChat({ chatId: isRemote ? chatId.replace(/^remote_/, '') : chatId, format: 'json', isRemote })
                  }>
                  Copy as JSON
                </ContextMenuItem>
                <ContextMenuItem
                  onClick={() =>
                    copyChat({ chatId: isRemote ? chatId.replace(/^remote_/, '') : chatId, format: 'text', isRemote })
                  }>
                  Copy as Text
                </ContextMenuItem>
              </ContextMenuSubContent>
...
function chatListSectionPropsAreEqual(prevProps: ChatListSectionProps, nextProps: ChatListSectionProps): boolean {
...
interface ChatListSectionProps {
  ...
  projectsMap: Map<
    string,
    { gitOwner?: string | null; gitProvider?: string | null; gitRepo?: string | null; name?: string | null }
  >;
...
const ArchiveButton = memo(
  forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(function ArchiveButton(props, ref) {
    return (
      <button
        ref={ref}
        type="button"
...
const ArchiveSection = memo(function ArchiveSection() {
  const [archivePopoverOpen, setArchivePopoverOpen] = useAtom(archivePopoverOpenAtom);
  ...
  // Block tooltip briefly after popover closes to avoid flicker
  useEffect(() => {
  ...
  const handleRestoreSuccess = useCallback(
    ...
    [setArchivePopoverOpen, setSelectedChatId, setSelectedChatIsRemote]
  );
  ...
  return (
    <Tooltip delayDuration={500} open={archivePopoverOpen || blockArchiveTooltip ? false : undefined}>
      <TooltipTrigger asChild>
        <div>
          <AgentsArchivePopover
            open={archivePopoverOpen}
            onOpenChange={setArchivePopoverOpen}
            onRestoreSuccess={handleRestoreSuccess}>
            <ArchiveButton ref={archiveButtonRef} suppressHydrationWarning />
          </AgentsArchivePopover>
        </div>
      </TooltipTrigger>
  ...
  return (
    <div
      ...
      <div
        ...
        <div className="text-sm font-semibold text-foreground truncate">Coder</div>
      </div>
...
export function AgentsSidebar({ onToggleSidebar, isMobileFullscreen = false, onChatSelect }: AgentsSidebarProps) {
  ...
  // Restore local workspace from archive (used by Cmd+Z undo)
  const restoreLocalChatMutation = trpc.chats.restore.useMutation({
    onSuccess: (chat) => {
      ...
      if (selectedChatId === variables.id) {
        ...
        if (autoAdvanceTarget === 'next') {
          ...
          const isPreviousAvailable =
            previousChatId && agentChats?.some((c) => c.id === previousChatId && c.id !== variables.id);
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'z' && undoStack.length > 0) {
        ...
        if (lastItem.type === 'workspace') {
          ...
          if (lastItem.isRemote) {
            ...
            restoreLocalChatMutation.mutate({ id: lastItem.chatId });
          }
          ...
          onSuccess: () => {
            ...
            if (selectedChatId === chatId) {
              ...
              if (autoAdvanceTarget === 'next') {
                ...
                const isPreviousAvailable =
                  previousChatId && agentChats?.some((c) => c.id === previousChatId && c.id !== chatId);
  ...
  useHotkeys(
    ...
    () => {
      ...
      const targetIndex =
        hoveredChatIndexRef.current >= 0 ? hoveredChatIndexRef.current : focusedChatIndex >= 0 ? focusedChatIndex : -1;
  ...
  useEffect(() => {
    const handleArchiveHotkey = (e: KeyboardEvent) => {
      ...
      const isDesktopShortcut = isDesktop && e.metaKey && e.code === 'KeyE' && !e.altKey && !e.shiftKey && !e.ctrlKey;
        ...
        <div
          ...
          {drafts.length > 0 && !searchQuery && (
            <div className={cn('mb-4', isMultiSelectMode ? 'px-0' : '-mx-1')}>
              <div className={cn('flex items-center h-4 mb-1', isMultiSelectMode ? 'pl-3' : 'pl-2')}>
                <h3 className="text-xs font-medium text-muted-foreground whitespace-nowrap">Drafts</h3>
              </div>
