 ...
 */
function SortableSubChatRow({ id, disabled, children }: { id: string; disabled?: boolean; children: React.ReactNode }) {
  ...
  return (
    <div
      ...
      className={cn('group/sortable', !disabled && (isDragging ? 'cursor-grabbing' : 'cursor-grab'))}
      {...attributes}
      {...listeners}>
      {!disabled && (
        <GripVertical
...
export function AgentsSubChatsSidebar({
  ...
  const [renamingSubChat, setRenamingSubChat] = useState<SubChatMeta | null>(null);
  ...
  const [subChatToArchive, setSubChatToArchive] = useState<SubChatMeta | null>(null);
  ...
  const openSubChats = useMemo(() => {
    const chats = openSubChatIds.map((id) => allSubChatsById.get(id)).filter((sc): sc is SubChatMeta => !!sc);
    ...
    return [...nonSplitChats.slice(0, insertAt), ...splitChats, ...nonSplitChats.slice(insertAt)];
  ...
  const { pinnedChats, unpinnedChats } = useMemo(() => {
    const filtered = searchQuery.trim()
      ? openSubChats.filter((chat) => chat.name.toLowerCase().includes(searchQuery.toLowerCase()))
      : openSubChats;
  ...
  const handleSubChatMouseEnter = useCallback((subChatId: string, name: string, cardElement: HTMLElement) => {
  ...
  }, [selectedSubChatIds, pinnedSubChatIds, togglePinSubChat, clearSubChatSelection]);
  ...
  useHotkeys(
    ...
    [filteredSubChats, focusedChatIndex, toggleSubChatSelection]
  );
  ...
  useHotkeys(
    ...
    [isMultiSelectMode, clearSubChatSelection, archiveAgentDialogOpen, renameDialogOpen]
  );
                  ...
                  {pinnedChats.length > 0 && (
                    <>
                      <div className={cn('flex items-center h-4 mb-1', isMultiSelectMode ? 'pl-3' : 'pl-2')}>
                        <h3 className="text-xs font-medium text-muted-foreground whitespace-nowrap">Pinned Chats</h3>
                      </div>
                      <div className="list-none p-0 m-0 mb-3">
                        <SortableContext items={pinnedChats.map((c) => c.id)} strategy={verticalListSortingStrategy}>
                          {pinnedChats.map((subChat) => {
                            ...
                            return (
                              <SortableSubChatRow key={subChat.id} id={subChat.id} disabled={isSplitTab}>
                                <ContextMenu>
                                  ...
                                  {isMultiSelectMode && selectedSubChatIds.has(subChat.id) ? (
                                    <ContextMenuContent className="w-48">
                                      {canShowPinOption && (
                                        <>
                                          <ContextMenuItem
                                            onClick={areAllSelectedPinned ? handleBulkUnpin : handleBulkPin}>
                                      ...
                                      <ContextMenuItem onClick={handleBulkArchive}>
                                        Archive {selectedSubChatIds.size} {pluralize(selectedSubChatIds.size, 'chat')}
                                      </ContextMenuItem>
                                    </ContextMenuContent>
                                  ) : (
                                    <SubChatContextMenu
                  ...
                  {unpinnedChats.length > 0 && (
                    <>
                      ...
                      <div className="list-none p-0 m-0">
                        <SortableContext items={unpinnedChats.map((c) => c.id)} strategy={verticalListSortingStrategy}>
        ...
        {isMultiSelectMode && (
          <motion.div
            ...
            <div className="flex items-center justify-between px-1">
              <span className="text-xs text-muted-foreground">{selectedSubChatsCount} selected</span>
              <button
      ...
      <AlertDialog
        ...
        <AlertDialogContent>
          ...
          <AlertDialogDescription className="px-5 pb-5">
            ...
            <span className="font-medium text-foreground">{agentName || subChatToArchive?.name || 'this agent'}</span>?
