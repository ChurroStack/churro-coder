    ...
    <div className="flex items-start gap-2.5">
      ...
      {isMultiSelectMode && !card.isDraft && (
        <div className="pt-0.5 flex-shrink-0">
          <Checkbox checked={card.isSelected} onClick={(e) => onCheckboxClick(e, card.chatId)} className="h-4 w-4" />
        </div>
      )}
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent className="w-48">
        ...
        {card.branch && <ContextMenuItem onClick={() => onCopyBranch(card.branch!)}>Copy branch name</ContextMenuItem>}
        <ContextMenuSub>
