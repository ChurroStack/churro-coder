...
import { IconDoubleChevronRight, CustomTerminalIcon, IconSidePeek, IconBottomPanel } from '@/components/ui/icons';
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="min-w-[140px]">
        {TERMINAL_MODES.map(({ value, label, Icon }) => (
          <DropdownMenuItem key={value} onClick={() => onModeChange(value)} className="flex items-center gap-2">
...
export function TerminalSidebar({
  ...
  const renameTerminal = useCallback(
    (id: string, name: string) => {
      ...
      setAllTerminals((prev) => ({
        ...prev,
        [currentScopeKey]: (prev[currentScopeKey] || []).map((t) => (t.id === id ? { ...t, name } : t))
      }));
    },
    [setAllTerminals]
  );
  ...
  return (
    <ResizableSidebar
      ...
      <div className="flex flex-col h-full min-w-0 overflow-hidden">
        {/* Header with tabs */}
        <div className="flex items-center gap-1 pl-1 pr-2 py-1.5 flex-shrink-0" style={{ backgroundColor: terminalBg }}>
    ...
    <div className="flex flex-col h-full min-w-0 overflow-hidden">
      ...
      <div className="flex-1 min-h-0 min-w-0 overflow-hidden" style={{ backgroundColor: terminalBg }}>
        {activeTerminal ? (
          ...
          <div className="flex items-center justify-center h-full text-muted-foreground text-sm">No terminal open</div>
        )}
      </div>
    </div>
  );
