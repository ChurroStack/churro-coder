...
function CommitHeader({ shortHash, message, date }: { shortHash: string; message: string; date: Date }) {
  return (
    <>
...
export function CommitItem({
  ...
  return (
    <CollapsibleRow
      ...
      header={<CommitHeader shortHash={commit.shortHash} message={commit.message} date={commit.date} />}>
      {hasFiles && (
        <FileList
          files={commit.files}
          viewMode={viewMode}
