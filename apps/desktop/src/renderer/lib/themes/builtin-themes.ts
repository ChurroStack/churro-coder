...
/**
 * ChurroStack Dark - Default dark theme matching the app's original design
 * Uses the brand blue (#0034FF) as primary/accent color
 */
const CHURROSTACK_DARK: VSCodeFullTheme = {
  id: "churrostack-dark",
  name: "ChurroStack Dark",
  type: "dark",
  source: "builtin",
  colors: {
...
/**
 * ChurroStack Light - Default light theme matching the app's original design
 * Uses the brand blue (#0034FF) as primary/accent color
 */
const CHURROSTACK_LIGHT: VSCodeFullTheme = {
  id: "churrostack-light",
  name: "ChurroStack Light",
  type: "light",
  source: "builtin",
  colors: {
...
export const BUILTIN_THEMES: VSCodeFullTheme[] = [
  // ChurroStack default themes (first)
  CHURROSTACK_DARK,
  CHURROSTACK_LIGHT,
  // Cursor themes
  CURSOR_DARK,
...
export function getBuiltinThemeById(id: string): VSCodeFullTheme | undefined {
  ...
  return BUILTIN_THEMES.find((theme) => theme.id === resolvedId)
}
 ...
 */
export const DEFAULT_LIGHT_THEME_ID = "churrostack-light"
export const DEFAULT_DARK_THEME_ID = "churrostack-dark"

/**
 * Legacy theme ID alias map — preserves user theme selection across the rebrand
 * from "21st-*" to "churrostack-*" so persisted preferences don't reset to default.
 */
export const LEGACY_THEME_IDS: Record<string, string> = {
  "21st-dark": "churrostack-dark",
  "21st-light": "churrostack-light",
}

/**
 * Resolve a possibly-legacy theme id to its current id.
 * Use at every theme-resolution call site so users with persisted "21st-*" ids
 * still get the renamed builtin theme.
 */
