/**
 * Pure rules for repository-name validation. Used by both the renderer
 * (synchronous client-side feedback) and the main process (re-validated server
 * side before `createProject` proceeds, with an additional path-existence
...
export function validateRepoNameRules(name: string, provider: 'github' | 'azure' | 'local'): NameValidationResult {
