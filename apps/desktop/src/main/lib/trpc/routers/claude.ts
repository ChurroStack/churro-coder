  ...
  try {
    ...
    if (settings?.activeAccountId) {
      ...
      if (account?.oauthToken) {
        console.log('[claude-auth] Using multi-account system, activeAccountId:', settings.activeAccountId);
        const decrypted = decryptToken(account.oauthToken);
        console.log('[claude-auth] Token decrypted successfully');
        console.log('[claude-auth] Token preview:', decrypted.slice(0, 20) + '...' + decrypted.slice(-10));
 ...
 */
async function readProjectMcpJsonCached(projectPath: string): Promise<Record<string, McpServerConfig>> {
  try {
    ...
    if (Object.keys(mergedGlobalServers).length > 0) {
      groupTasks.push({
        ...
        promise: (async () => {
          ...
          const freshServers = await ensureMcpTokensFresh(mergedGlobalServers, GLOBAL_MCP_PATH);
    ...
    .input(
      z.object({
        ...
        customConfig: z
          ...
          .optional(),
        effort: z.enum(['low', 'medium', 'high', 'xhigh', 'max']).optional(), // Thinking/reasoning effort level
          ...
          observable<UIMessageChunk>((emit) => {
            ...
            const safeEmit = (chunk: UIMessageChunk) => {
              ...
              if (chunk.type === 'text-delta') {
                ...
                if (pendingTextDeltaTimer === null) {
                  pendingTextDeltaTimer = setTimeout(flushPendingTextDelta, TEXT_DELTA_FLUSH_MS);
                }
            ...
            void Sentry.withActiveSpan(span, async () => {
              ...
              try {
                ...
                let existing = db.select().from(subChats).where(eq(subChats.id, input.subChatId)).get();
                ...
                if (!existing) {
                  console.warn(`[claude] sub-chat ${input.subChatId} missing on send — auto-creating`);
                  db.insert(subChats)
                    .values({
                      id: input.subChatId,
                      chatId: input.chatId,
                ...
                const lastAssistantMsg = [...existingMessages].reverse().find((m: any) => m.role === 'assistant');
                const resumeAtUuid = lastAssistantMsg?.metadata?.shouldResume
                  ? lastAssistantMsg?.metadata?.sdkMessageUuid || null
                  : null;
                const shouldForkResume = lastAssistantMsg?.metadata?.shouldForkResume === true;
                ...
                if (isUsingOllama) {
                  ...
                  const isDefaultAnthropicUrl =
                    !finalCustomConfig.baseUrl || finalCustomConfig.baseUrl.includes('anthropic.com');
                  ...
                  if (!symlinksCreated.has(cacheKey)) {
                    ...
                    const ensureSymlink = async (
                      ...
                      try {
                        ...
                        console.warn(`[claude] Failed to symlink ${label}:`, (symlinkErr as Error).message);
                      }
                    };

                    await ensureSymlink(skillsSource, skillsTarget, 'skills directory', 'dir');
                ...
                console.log('[claude-auth] hasExistingApiConfig:', hasExistingApiConfig);
                ...
                console.log('[claude-auth] Using ANTHROPIC_BASE_URL:', finalEnv.ANTHROPIC_BASE_URL || '(default)');
                ...
                const expectedSessionPath = resumeSessionId
                  ? path.join(isolatedConfigDir, 'projects', expectedSanitizedCwd, `${resumeSessionId}.jsonl`)
                  : null;
                if (resumeSessionId && expectedSessionPath && !existsSync(expectedSessionPath)) {
                  console.log(`[claude] Session file missing at ${expectedSessionPath} - starting fresh session`);
                ...
                if (
                  ...
                  console.log(`[claude] Mode changed (${existingSessionMode}→${input.mode}) - forcing fresh session`);
                  db.update(subChats)
                ...
                // the child env.
                const has1MSuffix = typeof rawResolvedModel === 'string' && rawResolvedModel.endsWith('[1m]');
                const resolvedModel = has1MSuffix ? rawResolvedModel!.slice(0, -4) : rawResolvedModel;
                if (has1MSuffix) {
                  ...
                  console.log(`[claude] 1M context enabled for ${resolvedModel} — ANTHROPIC_BETAS=${merged}`);
                }
                  ...
                  if (existingMessages.length > 0) {
                    ...
                    for (const msg of existingMessages) {
                      if (msg.role === 'user') {
                        ...
                        const textParts =
                          msg.parts?.filter((p: any) => p.type === 'text').map((p: any) => p.text) || [];
...
[/CURRENT REQUEST]`;
                      ...
                      if (sandboxOn) {
                        ...
                        if (readDenyTools.has(toolName)) {
                          ...
                          if (resolvedSp) {
                            ...
                            const insideHome = resolvedSp === home || resolvedSp.startsWith(home + path.sep);
                            if (insideHome && !pathIsInsideAny(resolvedSp, allowedRoots)) {
                              return {
                                behavior: 'deny',
                ...
                while (true) {
                  ...
                  const wedgeTimer = setTimeout(() => {
                    if (!firstMessageReceived) {
                      streamWedged = true;
                      console.error(`[claude] Stream wedged — no data in ${WEDGE_TIMEOUT_MS / 1000}s, aborting`);
                  ...
                  try {
                    for await (const msg of stream) {
                      ...
                      if (msgAny.session_id) {
                        const resolvedSessionId = msgAny.session_id as string;
                        const sessionChanged = currentSessionId !== resolvedSessionId;
                        metadata.sessionId = resolvedSessionId;
                        currentSessionId = resolvedSessionId; // Share with cleanup
                        if (sessionChanged) {
                          span.setAttribute('session_id', resolvedSessionId);
                          recordChatEvent({
                            ts: Date.now(),
                          ...
                          Sentry.logger.info(
                            `stream session resolved sub=${subId}`,
                            logAttributes({ session_id: resolvedSessionId })
                          );
                        }
            ...
            return () => {
              ...
              if (!emittedFinish) {
                console.log(`[SD] M:CLEANUP_SYNTHETIC_FINISH sub=${subId}`);
                safeEmit({ type: 'finish' } as UIMessageChunk);
              }
   ...
   */
  getMcpConfig: publicProcedure.input(z.object({ projectPath: z.string() })).query(async ({ input }) => {
    try {
      const config = await readClaudeConfig();
