      ...
      if (isDirectory) {
        ...
        try {
          ...
          if (source === 'project' && basePath) {
            ...
            displayPath = fullPath.startsWith(homeDir) ? '~' + fullPath.slice(homeDir.length) : fullPath;
          }
  ...
  list: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input?.projectPath) {
        ...
        projectCommandsPromise = scanCommandsDirectory(projectCommandsDir, 'project', '', input.projectPath);
      }
