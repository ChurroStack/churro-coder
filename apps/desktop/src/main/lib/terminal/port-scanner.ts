...
async function getProcessNameWindowsAsync(pid: number): Promise<string> {
  ...
  let name = 'unknown';
  try {
    const { stdout: output } = await execFileAsync('wmic', ['process', 'where', `processid=${pid}`, 'get', 'name'], {
      timeout: 2000
    });
    const lines = output.trim().split('\n');
    if (lines.length >= 2) {
...
export async function getProcessName(pid: number): Promise<string> {
  ...
  if (platform === 'win32') {
    ...
    // macOS/Linux
    try {
      const { stdout: output } = await execFileAsync('sh', ['-c', `ps -p ${pid} -o comm= 2>/dev/null || true`], {
        timeout: 2000
      });
