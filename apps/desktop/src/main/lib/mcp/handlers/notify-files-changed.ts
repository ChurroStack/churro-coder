...
export function registerNotifyFilesChangedTool(server: McpServer): void {
  server.registerTool(
    'notify_files_changed',
    {
      title: 'Notify Files Changed',
      description:
        'Report every file you create, modify, or delete during this session. Call this immediately after any successful write/edit/delete (batch multiple files in one call if you did them together). The host app uses these paths to attribute changes to this sub-chat in the Changes widget. You MUST pass subChatId, which the host app provides in the prompt context (look for "Sub-chat id: <value>").',
      inputSchema: {
        subChatId: z
          .string()
          .min(1)
    ...
    async (rawInput: Record<string, unknown>) => {
      ...
      console.log(
        `[churro-coder] notify_files_changed called sub=${id ?? 'missing'} inputKeys=${inputKeys} count=${input.files?.length ?? 0}`
      );
