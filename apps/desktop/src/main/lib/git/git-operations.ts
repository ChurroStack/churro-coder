...
import { isUpstreamMissingError, isNonFastForwardPushError, REMOTE_AHEAD_ERROR_PREFIX } from './git-utils';
    ...
    fetch: publicProcedure
      ...
      .mutation(async ({ input }): Promise<{ success: boolean }> => {
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          await withLockRetry(input.worktreePath, () => git.fetch(['--all', '--prune']));
    ...
    checkout: publicProcedure
      .input(
        z.object({
          ...
          uncommittedStrategy: z.enum(['abort', 'carry', 'stash']).optional().default('abort')
        })
      )
      .mutation(async ({ input }): Promise<{ success: boolean; stashPopFailed?: boolean }> => {
        assertRegisteredWorktree(input.worktreePath);
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          if (dirty && input.uncommittedStrategy === 'stash') {
            await withLockRetry(input.worktreePath, () =>
              git.stash(['push', '-u', '-m', `Auto-stash before switching to ${input.branch}`])
            );
          }

          try {
            await withLockRetry(input.worktreePath, () => git.checkout(input.branch));
          } catch (checkoutError) {
    ...
    getHistory: publicProcedure
      ...
      .query(
        ...
        > => {
          ...
          const git = createGit(input.worktreePath);
          const logOutput = await git.raw(['log', `-${input.limit}`, '--format=%H|%h|%s|%an|%ae|%aI|%D']);
          ...
          return logOutput
            ...
            .split('\n')
            .map((line) => {
              const [hash, shortHash, message, author, email, dateStr, refs] = line.split('|');
    ...
    atomicCommit: publicProcedure
      ...
      .mutation(async ({ input }): Promise<{ success: boolean; hash: string }> => {
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          await withLockRetry(input.worktreePath, () => git.reset(['HEAD']));

          // Stage only the selected files
          await withLockRetry(input.worktreePath, () => git.add(['--', ...input.filePaths]));
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          try {
            ...
            if (isNonFastForwardPushError(message)) {
              throw new Error(`${REMOTE_AHEAD_ERROR_PREFIX} Remote has new commits. Pull with rebase and retry.`);
            }
            throw error;
          }
          await git.fetch();
      ...
      .mutation(async ({ input }): Promise<{ success: boolean }> => {
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          try {
            ...
            await withLockRetry(input.worktreePath, () => git.pull(['--rebase']));
    ...
    forcePush: publicProcedure
      ...
      .mutation(async ({ input }): Promise<{ success: boolean }> => {
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          await withLockRetry(input.worktreePath, () => git.push(['--force-with-lease']));
          await git.fetch();
          invalidateGitStateCaches(input.worktreePath);
          return { success: true };
        });
    ...
    mergeFromDefault: publicProcedure
      ...
      .mutation(async ({ input }): Promise<{ success: boolean }> => {
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          await withLockRetry(input.worktreePath, () => git.fetch(['--all']));
          ...
          try {
            if (input.useRebase) {
              await withLockRetry(input.worktreePath, () => git.rebase([`origin/${defaultBranch}`]));
            } else {
              await withLockRetry(input.worktreePath, () => git.merge([`origin/${defaultBranch}`, '--no-edit']));
            }
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
    ...
    createPR: publicProcedure
      ...
      .mutation(async ({ input }): Promise<{ success: boolean; url: string }> => {
        ...
        return withGitLock(input.worktreePath, async () => {
          ...
          if (!hasUpstream) {
            await withLockRetry(input.worktreePath, () => git.push(['--set-upstream', 'origin', branch]));
          } else {
          ...
          if (!provider) {
            throw new Error('Could not determine repository provider from remote URL');
          }
