...
export async function createWorktree(
  ...
  try {
    ...
    await execFileAsync('git', ['-C', mainRepoPath, 'worktree', 'add', worktreePath, '-b', branch, commitHash], {
      env,
      timeout: 120_000
    });
  } catch (error) {
...
export function isPathInsideWorktreeRoot(workspacePath: string): boolean {
  const roots = [
    resolve(join(homedir(), '.churrostack', 'worktrees')) + sep,
    resolve(join(homedir(), '.21st', 'worktrees')) + sep
  ];
...
export async function removeWorktree(
  ...
  if (isPathInsideWorktreeRoot(worktreePath)) {
    ...
    console.warn(
      `[Worktree] Refusing to rmdir path outside ~/.churrostack/worktrees (and legacy ~/.21st/worktrees): ${worktreePath}`
    );
  }
...
export async function worktreeExists(mainRepoPath: string, worktreePath: string): Promise<boolean> {
  try {
    const git = simpleGit(mainRepoPath);
...
export async function fetchDefaultBranch(mainRepoPath: string, defaultBranch: string): Promise<string> {
  const git = simpleGit(mainRepoPath);
 ...
 */
export async function refreshDefaultBranch(mainRepoPath: string): Promise<string | null> {
  const git = simpleGit(mainRepoPath);
...
export async function checkNeedsRebase(worktreePath: string, defaultBranch: string): Promise<boolean> {
  const git = simpleGit(worktreePath);
  const behindCount = await git.raw(['rev-list', '--count', `HEAD..origin/${defaultBranch}`]);
...
export type BranchExistsResult = { status: 'exists' } | { status: 'not_found' } | { status: 'error'; message: string };
...
export async function branchExistsOnRemote(worktreePath: string, branchName: string): Promise<BranchExistsResult> {
  const env = await getGitEnv();

  try {
    // Use execFileAsync directly to get reliable exit codes
    // simple-git doesn't expose exit codes in a predictable way
    await execFileAsync('git', ['-C', worktreePath, 'ls-remote', '--exit-code', '--heads', 'origin', branchName], {
      env,
      timeout: 30_000
    });
 ...
 */
export async function checkBranchCheckoutSafety(repoPath: string): Promise<CheckoutSafetyResult> {
  const git = simpleGit(repoPath);
...
export async function safeCheckoutBranch(repoPath: string, branch: string): Promise<void> {
  ...
  try {
    ...
    if (originPresent) {
      try {
        ...
        console.warn(`[worktree] Pre-create fetch of origin/${baseBranch} failed: ${msg}`);
      }
    }
...
export async function getWorktreeDiff(
  ...
  try {
    ...
    if (!status.isClean()) {
      const exclusionArgs = [':!*.lock', ':!*-lock.*', ':!package-lock.json', ':!pnpm-lock.yaml', ':!yarn.lock'];
