...
const CSCODE_CONFIG_PATH = ".cscode/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .cscode/worktree.json > .1code/worktree.json (legacy, read-only)
 */
export async function detectWorktreeConfig(
  ...
  // 3. Check .cscode/worktree.json
  const cscodePath = join(projectPath, CSCODE_CONFIG_PATH)
  if (await fileExists(cscodePath)) {
    const config = await readJsonFile<WorktreeConfig>(cscodePath)
    if (config) {
      return { config, path: cscodePath, source: "cscode" }
    }
  }

  // 4. Legacy fallback: .1code/worktree.json (read-only; saves go to new path)
  ...
  if (await fileExists(legacyPath)) {
    ...
    if (config) {
      return { config, path: legacyPath, source: "cscode" }
    }
  }
...
export async function getAvailableConfigPaths(
  ...
  cscode: { exists: boolean; path: string }
}> {
  const cursorPath = join(projectPath, CURSOR_CONFIG_PATH)
  const cscodePath = join(projectPath, CSCODE_CONFIG_PATH)
  ...
  return {
    ...
    cscode: {
      exists: await fileExists(cscodePath),
      path: cscodePath,
    },
  }
}
...
export async function saveWorktreeConfig(
  ...
  target: "cursor" | "cscode" | string = "cscode",
): Promise<{ success: boolean; path: string; error?: string }> {
  let targetPath: string

  if (target === "cursor") {
    targetPath = join(projectPath, CURSOR_CONFIG_PATH)
  } else if (target === "cscode") {
    targetPath = join(projectPath, CSCODE_CONFIG_PATH)
  } else {
    // Custom path
