...
export class WindowsPlatformProvider extends BasePlatformProvider {
  ...
  getCliConfig(): CliConfig {
    ...
    return {
      installPath: path.join(home, ".local", "bin", "cscode.cmd"),
      scriptName: "cscode.cmd",
  ...
  async installCli(
    ...
    try {
      ...
      console.log("[CLI] Installed cscode command to", installPath)
      console.log(
      ...
      return {
        success: true,
        pathHint: `To use cscode from terminal, add to your PATH: ${installDir}`,
      }
    } catch (error: unknown) {
      const errorMessage =
  ...
  async uninstallCli(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      console.log("[CLI] Uninstalled cscode command")
      return { success: true }
    } catch (error: unknown) {
      const errorMessage =
